    ...
    def export_as_markdown(
        ...
        try:
            ...
            if "plan" in content:
                ...
                lines.append(f"**Narrative Frame:** {plan.get('narrativeFrame', 'N/A')}\n")
            ...
            if "outputs" in content:
                ...
                platforms_to_export = [platform_filter] if platform_filter != "all" else platforms
                
                for platform in platforms_to_export:
                    if platform in outputs:
                        lines.append(f"## {platform.capitalize()} Output\n")
                        lines.append(self._format_platform_output(platform, outputs[platform]))
                        lines.append("")
    ...
    def export_as_json(
        ...
        try:
            ...
            export_data = {
                "id": content_id,
                "topic": topic,
                "platforms": platforms if platform_filter == "all" else [platform_filter],
                "content": {
    ...
    def _format_platform_output(self, platform: str, output: dict) -> str:
        ...
        try:
            if platform == "twitter" and "tweets" in output:
                lines.append(f"**Thread Structure:** {output.get('threadStructure', 'N/A')}\n")
                lines.append("**Tweets:**\n")
                for tweet in output["tweets"]:
                    lines.append(f"{tweet['order']}. {tweet['content']} ({tweet['characterCount']} chars)")
                ...
                if "carousel" in output and output["carousel"]:
                    ...
                    for slide in output["carousel"]["slides"]:
                        lines.append(f"**Slide {slide['slideNumber']}: {slide['title']}**")
            ...
            logger.warning("Error formatting platform output", platform=platform, error=str(e))
            lines.append(json.dumps(output, indent=2))
