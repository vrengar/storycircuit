...
class AgentService:
    ...
    def _get_project_client(self) -> AIProjectClient:
        ...
        if self._project_client is None:
            logger.info("Initializing Azure AI Project Client", endpoint=self.settings.azure_ai_endpoint)
    ...
    def _get_agent(self):
        ...
        if self._agent is None:
            ...
            logger.info("Getting agent from Foundry by name", agent_name=self.settings.agent_name)
            try:
                # Get agent by name - works for new Foundry agents
                self._agent = client.agents.get(agent_name=self.settings.agent_name)
                logger.info("Agent retrieved successfully", agent_name=self._agent.name, agent_id=self._agent.id)
                    
            except Exception as e:
                logger.error("Failed to get agent", agent_name=self.settings.agent_name, error=str(e))
                raise AgentServiceError(f"Failed to get agent '{self.settings.agent_name}': {str(e)}")
        return self._agent
    ...
    async def generate_content(
        ...
        try:
            ...
            logger.error("Unexpected error during content generation", error=str(e), error_type=type(e).__name__)
    ...
    def _parse_agent_response(self, content_text: str) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                plan_section_match = re.search(r'##\s*A\)\s*Plan\s*\n(.*?)(?:##\s*B\)|$)', content_text, re.IGNORECASE | re.DOTALL)
                plan_section = plan_section_match.group(1) if plan_section_match else content_text
                
                # Extract hook - format: **Hook:**  \nContent text
                hook = content_text[:250].split('\n')[0] if content_text else "Generated content available below"
                hook_match = re.search(r'\*\*Hook:\*\*\s*\n(.+?)(?:\n\s*\n|\*\*)', plan_section, re.IGNORECASE | re.DOTALL)
                if hook_match:
                    hook = hook_match.group(1).strip()
                
                # Extract narrative frame - format: **Narrative Frame:**  \nContent
                narrative = "Structured content framework provided"
                narrative_match = re.search(r'\*\*Narrative\s*Frame:\*\*\s*\n(.+?)(?:\n\s*\n|\*\*)', plan_section, re.IGNORECASE | re.DOTALL)
                if narrative_match:
                    narrative = narrative_match.group(1).strip()
                
                # Extract key points - split approach
                key_points = ["Full detailed content available in the Notes section below"]
                # Split on "Key Points:" and get the section after it
                if '**Key Points:**' in plan_section or '**Key Points**' in plan_section:
                    # Find the section after Key Points
                    kp_split = re.split(r'\*\*Key\s*Points?:?\*\*', plan_section, flags=re.IGNORECASE)
                    if len(kp_split) > 1:
                        kp_section = kp_split[1]
                        # Extract until the next heading or end
                        kp_text_match = re.search(r'^(.+?)(?:\n\s*-\s*\*\*|\n\s*##|$)', kp_section, re.DOTALL)
                        if kp_text_match:
                            kp_text = kp_text_match.group(1)
                            # Now extract all bullet points (with or without indentation)
                            extracted_points = re.findall(r'^\s*[-•]\s+(.+?)$', kp_text, re.MULTILINE)
                            if extracted_points:
                                key_points = [p.strip() for p in extracted_points if p.strip()]
                ...
                cta_match = re.search(r'\*\*(?:CTA|Call[- ]to[- ]Action):\*\*\s*\n(.+?)(?:\n\s*\n|\*\*|---+|$)', plan_section, re.IGNORECASE | re.DOTALL)
                if cta_match:
                    cta = cta_match.group(1).strip()
                
                logger.info("Extracted structured fields",
                ...
                outputs = {}
                platform_section_match = re.search(r'##\s*B\)\s*(?:PLATFORM\s*)?OUTPUTS\s*\n(.*?)(?:##\s*C\)|$)', content_text, re.IGNORECASE | re.DOTALL)
                
                if platform_section_match:
                    platform_section = platform_section_match.group(1)
                    logger.info("Found PLATFORM OUTPUTS section", section_length=len(platform_section))
                    ...
                    for platform in ["linkedin", "twitter", "github", "blog"]:
                        ...
                        platform_match = re.search(platform_pattern, platform_section, re.IGNORECASE | re.DOTALL)
                        ...
                        if platform_match:
                            ...
                            hashtags_match = re.search(hashtags_pattern, platform_section, re.IGNORECASE | re.DOTALL)
                            hashtags = []
                            if hashtags_match:
                            ...
                            cta_match = re.search(cta_pattern, platform_section, re.IGNORECASE | re.DOTALL)
                            platform_cta = cta_match.group(1).strip() if cta_match else cta
                            ...
                            logger.info(f"Extracted {platform} content", 
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            logger.info("Health check completed", healthy=is_healthy, agent_name=agent.name if agent else None)
            return is_healthy
        except Exception as e:
