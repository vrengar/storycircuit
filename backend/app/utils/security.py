...
SENSITIVE_PATTERNS = {
    ...
    "secret": re.compile(
        r'(secret|token|auth)[\'"\s:=]+[a-zA-Z0-9_-]{20,}', re.IGNORECASE
    ),
    "connection_string": re.compile(
        r"(DefaultEndpointsProtocol|AccountKey|SharedAccessSignature)=", re.IGNORECASE
    ),
    ...
    "confidential_marker": re.compile(
        r"\b(confidential|internal|proprietary|msft\s+confidential)\b", re.IGNORECASE
    ),
}
...
class ContentSecurityValidator:
    ...
    def sanitize_user_input(text: str) -> str:
        ...
        # Remove credit cards
        sanitized = re.sub(
            SENSITIVE_PATTERNS["credit_card"], "[CARD-REMOVED]", sanitized
        )
